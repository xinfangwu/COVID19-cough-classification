...
def train_loop(model, loader, optimizer, loss_func):
    model.train()
    ...
    for i, batch in tqdm(enumerate(loader, 1), position=1, total=len(loader)):
        ...
        outputs = model.forward(batch['img_spec'])
...
if __name__ == '__main__':
    ...
    for iep in trange(1, config.training.epoch+1, position=0):
        ...
        # validating
        vep_loss = valid_loop(net, valid_loader, config.model.loss_func)
        print(f'EP[{iep}/{config.training.epoch}] valid:  ' +
              ' \ '.join([f'{k} {v:.3f}' for k, v in vep_loss.items()]))
